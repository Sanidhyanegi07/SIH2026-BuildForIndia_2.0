...
class AdminDashboardActivity : AppCompatActivity() {
    ...
    private enum class AdminTab { EMERGENCIES, HAZARDS, INCIDENTS, EVENTS, ROUTES, RESOURCES, VERIFICATIONS, SETTINGS }
    ...
    private fun setupTabs() {
        ...
        val btnEvents = findViewById<Button>(R.id.buttonTabEvents)
        val btnRoutes = findViewById<Button>(R.id.buttonTabRoutes)
        ...
        val btnSettings = findViewById<Button>(R.id.buttonTabSettings)
        ...
        fun updateTabStyles() {
            ...
            btnEvents.background = if (currentTab == AdminTab.EVENTS) violetBg else secondaryBg
            btnEvents.setTextColor(if (currentTab == AdminTab.EVENTS) white else textPrimary)

            btnRoutes.background = if (currentTab == AdminTab.ROUTES) violetBg else secondaryBg
            btnRoutes.setTextColor(if (currentTab == AdminTab.ROUTES) white else textPrimary)
            ...
            btnSettings.background = if (currentTab == AdminTab.SETTINGS) violetBg else secondaryBg
            btnSettings.setTextColor(if (currentTab == AdminTab.SETTINGS) white else textPrimary)
        ...
        btnEvents.setOnClickListener { currentTab = AdminTab.EVENTS; updateTabStyles() }
        btnRoutes.setOnClickListener { currentTab = AdminTab.ROUTES; updateTabStyles() }
        ...
        btnSettings.setOnClickListener { currentTab = AdminTab.SETTINGS; updateTabStyles() }
    }
    ...
    private fun setupActionButtons() {
        ...
        findViewById<Button>(R.id.buttonAdminUsers).setOnClickListener {
        ...
        when (currentTab) {
            ...
            AdminTab.INCIDENTS -> {
                ...
                val createBtn = Button(this).apply {
                    ...
                    setOnClickListener { showCreateIncidentDialog() }
                }
            ...
            AdminTab.ROUTES -> {
                sectionTitle.text = "ROUTE INSPECTION & IMPACTS"
...
AdminTab.VERIFICATIONS -> {
    ...
    private fun showCreateIncidentDialog() {
        ...
        val types = listOf("FLOODING", "LANDSLIDE", "ROAD_BLOCKAGE", "ACCIDENT", "EMERGENCY_EVENT")
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Create") { _, _ ->
                ...
                val stateId = states[stateSpinner.selectedItemPosition].first
                val districtSel = districtSpinner.selectedItem?.toString()
                val district = if (districtSel == "All districts") null else districtSel
                val incidentId = "incident_${System.currentTimeMillis()}"
                val payload = mapOf(
                    "type" to type,
                    "description" to descInput.text.toString().trim().ifBlank { "Reported by admin" },
                    "stateId" to stateId,
                ...
                lifecycleScope.launch {
                    try {
                        database.getReference("incidents").child(incidentId)
                            .setValue(payload).await()
                        Toast.makeText(this@AdminDashboardActivity,
                            "Incident created", Toast.LENGTH_SHORT).show()
                    } catch (e: Exception) {
                        Toast.makeText(this@AdminDashboardActivity,
                            "Failed to create incident: ${e.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            }
            .setNegativeButton("Cancel", null)
    ...
    /**
     * §49: builds a card for an event with tap-to-update-status.
     */
    private fun buildEventCard(event: EventItem): View {
        ...
        card.findViewById<TextView>(R.id.textAlertDate).text = "📅 ${event.type} · ${event.status}"
    ...
    /**
     * §49: dialog to update event status.
     */
    ...
    /**
     * §Admin Settings: admin-configurable preferences for the portal.
     */
    private fun renderRoutesTab(container: LinearLayout) {
        container.removeAllViews()

        val appContainer = (application as GeoRescuXApplication).appContainer
        val graph = try { appContainer.routeRepository.getGraph() } catch (e: Exception) { null }

        // §50: network health summary
        val summaryCard = layoutInflater.inflate(R.layout.item_alert, container, false)
        summaryCard.findViewById<TextView>(R.id.textAlertDate).text = "🛣️ Route Network Inspection"
        summaryCard.findViewById<TextView>(R.id.textAlertDuration).text = buildString {
            if (graph == null) {
                append("Route graph unavailable for the active region.")
            } else {
                append("Nodes: ${graph.nodes.size}\n")
                append("Road segments: ${graph.edges.size}\n")
                val blockedEdges = graph.edges.count { it.blocked }
                append("Blocked segments (graph): $blockedEdges\n")
                append("Hazards (graph): ${graph.hazards.size}\n")
                append("Blocked roads (admin ops): ${blockedRoadList.size}\n")
                append("Hazards (admin ops): ${hazardList.size}")
            }
        }
        summaryCard.findViewById<TextView>(R.id.textAlertLocation).text =
            "Local offline routing graph ${if (graph != null) "loaded" else "not installed"}"
        container.addView(summaryCard)

        // §50: blocked roads managed through the admin portal
        if (blockedRoadList.isNotEmpty()) {
            val header = TextView(this).apply {
                text = "BLOCKED ROADS (${blockedRoadList.size})"
                setTextColor(ContextCompat.getColor(this@AdminDashboardActivity, R.color.text_error))
                textSize = 12f
                setPadding(0, 24, 0, 8)
            }
            container.addView(header)
            blockedRoadList.forEach { block -> container.addView(buildBlockedRoadCard(block)) }
        }

        // §50: hazards that penalize/impact routing
        if (hazardList.isNotEmpty()) {
            val header = TextView(this).apply {
                text = "ROUTE-AFFECTING HAZARDS (${hazardList.size})"
                setTextColor(ContextCompat.getColor(this@AdminDashboardActivity, R.color.text_muted))
                textSize = 12f
                setPadding(0, 24, 0, 8)
            }
            container.addView(header)
            hazardList.forEach { hazard -> container.addView(buildHazardCard(hazard)) }
        }

        if (blockedRoadList.isEmpty() && hazardList.isEmpty() && graph != null) {
            val clear = TextView(this).apply {
                text = "No route impacts recorded. All inspected segments are passable."
                setTextColor(ContextCompat.getColor(this@AdminDashboardActivity, R.color.text_muted))
                textSize = 13f
    ...
    private fun renderSettingsTab(container: LinearLayout) {
        ...
        card.findViewById<TextView>(R.id.textAlertDate).text = "⚙️ Admin Settings"
        card.findViewById<TextView>(R.id.textAlertDuration).text = buildString {
            append("🔧 Sync interval: ${adminSettings.syncIntervalMinutes} min (configurable)\n")
            append("📊 Data retention: ${adminSettings.dataRetentionDays} days (configurable)\n")
            append("🔔 Alert TTL: ${adminSettings.alertTTLDays} days (configurable)\n")
            append("🌐 Region scope: ${selectedRegion?.displayName ?: "All"}\n")
            append("🔐 Admin role: Custom Claims verified")
        }
        card.findViewById<TextView>(R.id.textAlertLocation).apply {
