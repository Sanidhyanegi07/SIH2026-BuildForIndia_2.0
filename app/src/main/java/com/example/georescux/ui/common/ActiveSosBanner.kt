 ...
 * screen. The banner does not own emergency truth — it reads the active
 * emergency from the SOS repository on each refresh, so it cannot drift out
 * of sync if the emergency is ended elsewhere.
 *
...
object ActiveSosBanner {
    ...
    fun refresh(activity: Activity) {
        val banner = activity.findViewById<TextView>(R.id.activeSosBanner) ?: return
        ...
        banner.text = "🚨 SOS ACTIVE — tap to open emergency"
